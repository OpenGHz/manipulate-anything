...
class M2T2Agent(Agent):
    ...
    def rotate_gripper_z_45(self, current_orientation):
        """
        Rotate the robot gripper by 45 degrees along the z-axis.

        Parameters:
        - current_orientation: The current orientation of the gripper in quaternion form [w, x, y, z]

        Returns:
        - new_orientation: The new orientation of the gripper after the 45-degree rotation in quaternion form [w, x, y, z]
        """
        # Convert 45 degrees to radians for yaw
        theta = np.radians(45)  
        q_z_45 = [np.cos(theta/2), 0, 0, np.sin(theta/2)]

        # Multiply the current orientation with the 45-degree rotation quaternion along z-axis
        new_orientation = self.quaternion_multiply(current_orientation, q_z_45)
    ...
    def retract(self, obs):

        action = obs['gripper_matrix']
        message = ""

        if self.prev_state == 'retract':
            pred = np.array([
            ...
            action = pred  # if you intend to override the action with pred
            gripper_open = 1
            message = 'Double retract detected. Reset.'
        else:
            action[:3, 3] = action[:3, 3] - 0.05 * action[:3, 2]
            gripper_open = obs.get('gripper_open', 0)  # default to 0 if 'gripper_open' is not in obs

        return action, gripper_open, message
    ...
    def act(self, obs, state):
        ...
        # Roll
            ...
            #     message = 'Double retract detected. Reset.'
            # else:
            action[:3, 3] = action[:3, 3] + 0.01 * action[:3, 2]
            gripper_open = obs['gripper_open']
            message = 'Retract'
            pred = gripper_pose_from_rlbench(action)
            trans = action[:3, 3]
